...
class EBEC_Feedback
{
	...
	/**
	 * @param string|null $plugin_url     Plugin base URL (defaults to EBEC_URL when defined).
	 * @param string|null $plugin_version Plugin version (defaults to EBEC_VERSION when defined).
	 */
	public function __construct($plugin_url = null, $plugin_version = null)
	{
		$this->plugin_url     = null !== $plugin_url ? $plugin_url : (defined('EBEC_URL') ? EBEC_URL : '');
		$this->plugin_version = null !== $plugin_version ? $plugin_version : (defined('EBEC_VERSION') ? EBEC_VERSION : '');
	...
	/**
	 * Shared deactivation reason list (popup + submit handler).
	 */
...
<?php
	...
	/**
	 * Collect anonymous site/plugin details for feedback.
	 *
	 * @return array
	 */
	public function cpfm_get_user_info()
	{
		global $wpdb;
		...
		foreach ($active_plugins as $plugin_path) {
			...
			$author_url    = (! empty($plugin_info['AuthorURI'])) ? esc_url($plugin_info['AuthorURI']) : 'N/A';
			$plugin_url    = (! empty($plugin_info['PluginURI'])) ? esc_url($plugin_info['PluginURI']) : 'N/A';
			$plugin_data[] = array(
