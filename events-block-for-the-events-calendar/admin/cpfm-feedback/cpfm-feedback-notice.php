...
class CPFM_Feedback_Notice {
    ...
    public function cpfm_enqueue_assets() {
       ...
       $current_page   = isset($_GET['page'])? sanitize_key($_GET['page']):'';
        ...
        wp_enqueue_style(
            ...
            EBEC_URL . 'admin/cpfm-feedback/css/cpfm-admin-feedback.css',
            [],
            EBEC_VERSION
        );
        ...
        wp_enqueue_script(
            ...
            EBEC_URL . 'admin/cpfm-feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
            EBEC_VERSION, 
            true
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'events-block-for-the-events-calendar') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'events-block-for-the-events-calendar') . '</a></p>';
            $output .= '</div>';
            
            $output .= '<div class="cpf-extra-info">';
            $output .= '<p>' . esc_html__('Opt in to receive email updates about security improvements, new features, helpful tutorials, and occasional special offers. We\'ll collect:', 'events-block-for-the-events-calendar') . '</p>';
            $output .= '<ul>';
            $output .= '<li>' . esc_html__('Your website home URL and WordPress admin email.', 'events-block-for-the-events-calendar') . '</li>';
            $output .= '<li>' . esc_html__('To check plugin compatibility, we will collect the following: list of active plugins and themes, server type, MySQL version, WordPress version, memory limit, site language and database prefix. ', 'events-block-for-the-events-calendar');
            $output .= '<a href="https://my.coolplugins.net/terms/usage-tracking/" target="_blank">' . esc_html__('Click here', 'events-block-for-the-events-calendar') . '</a></li>';
            $output .= '</ul>';
            
            $output .= '</div>';
            
            $output .= '<div class="notice-actions">';
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'events-block-for-the-events-calendar') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'events-block-for-the-events-calendar') . '</button>';
            $output .= '</div>';
