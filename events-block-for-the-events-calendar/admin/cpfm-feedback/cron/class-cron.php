...
if (!class_exists('EBEC_cronjob')) {
    class EBEC_cronjob
    {
       ...
       static public function ebec_send_data() {
            ...
            $initial_version = get_option('ebec_initial_save_version');
            $initial_version = is_string($initial_version) ? sanitize_text_field($initial_version) : 'N/A';
            $plugin_version = defined('EBEC_VERSION') ? EBEC_VERSION : 'N/A';
            ...
            $post_data = array(
                ...
                'plugin_name'       => 'Events Block For The Events Calendar',
            ...
            if (is_wp_error($response)) {
                ...
                error_log('EBEC Feedback Send Failed: ' . $response->get_error_message());
                return;
            }
         ...
         */
        public function ebec_cron_schedules($schedules)
        {
